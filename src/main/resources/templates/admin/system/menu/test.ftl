...
<script type="text/javascript">
    layui.use(['tree', 'layer','table'], function() {
        ...
        $(function(){
            $.post("${base}/finance/sysMenu/treelist",function(res){
                if(res.success){
        ...
        var active={
            addUser : function(){
                var addIndex = layer.open({
                    title : "添加系统菜单",
                    type : 2,
                    content : "${base}/finance/admin/system/menu/add",
    ...
    var addChildMenu = function(data){
        var addIndex = layer.open({
            title : "添加系统菜单",
            type : 2,
            content : "${base}/finance/admin/system/menu/add?parentId="+data,
            success : function(layero, addIndex){
                setTimeout(function(){
                    layer.tips('点击此处返回角色列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    var editChildMenu = function(data){
        var editIndex = layer.open({
            title : "编辑菜单",
            type : 2,
            content : "${base}/finance/admin/system/menu/edit?id="+data,
    ...
    var delMenu =function(data){
        layer.confirm("你确定要删除该菜单么？这将会使得其下的所有子菜单都删除",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("${base}/finance/admin/system/menu/delete",{"id":data},function (res){
                        if(res.success){
