...
<head>
    ...
    <link rel="stylesheet" href="${base}/finance/static/zTree/v3/css/zTreeStyle/zTreeStyle.css" type="text/css">
    <style type="text/css">
        ...
        */
        ...
        .multiSelect a i{ float:left; display:block; margin:2px 0 0 2px; border-radius:2px; width:8px; height:8px; background:url(${base}/finance/static/images/close.png) no-repeat center; background-size:65%; padding:4px;}
...
<script type="text/javascript" src="${base}/finance/static/zTree/v3/js/jquery.ztree.all-3.5.min.js"></script>
<script>
    layui.use(['form','jquery','layer','upload','laydate'],function(){
        ...
        $(".layui-btn-xs").on("click",function () {
            ...
            layer.prompt({title: '输入一个标签的名字', formType: 0}, function(pass, index,elem){
                ...
                if(!reg.test(pass)){
                    ...
                    $.post("${base}/finance/admin/blogTags/checkTagName",{name:pass.trim()},function (res) {
                        if(res.success){
        ...
        var content_editor = new E('#content');
            //图片上传
        content_editor.customConfig.uploadImgServer = '${base}/finance/file/uploadWang';
        ...
        content_editor.customConfig.pasteTextHandle = function (content) {
            ...
            $.ajax({
                url: "${base}/finance/file/doContent/",
                type: "POST",
                async: false,
        ...
        var submitForm = function (data) {
            ...
            $.ajax({
                url:"${base}/finance/admin/blogArticle/edit",
                type:"POST",
