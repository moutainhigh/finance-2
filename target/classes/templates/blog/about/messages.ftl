...
<head>
    <#include "${base}/finance/blog/common/header.ftl">
    <!-- 本页样式表 -->
    <link href="${base}/finance/static/blog/css/about.css?t=${.now?long}" rel="stylesheet" />
    <style type="text/css">
        /**管理员回复的图片样式**/
        .admincss img{
...
<body>
<!-- 导航 -->
<#include "${base}/finance/blog/common/nav.ftl">
<!-- 主体（一般只改变这里的内容） -->
<div class="blog-body">
    <div class="blog-container">
        <blockquote class="layui-elem-quote sitemap layui-breadcrumb shadow" style="border-left: 5px solid #37C6C0;">
            <a class="blog-logo" href="${base}/finance/showBlog/index">${site.name}</a>
            <@articleChannelList cid="${channel.id}">
                <#list result as item>
                    <a href="${base}/finance/showBlog${item.href}"><cite>${item.name}</cite></a>
                </#list>
            </@articleChannelList>
        </blockquote>
        <div class="blog-main">
                ...
                <div class="layui-tab-content">
                    <div class="aboutinfo">
                        <div class="aboutinfo-figure">
                            <img src="${base}/finance/static/blog/images/messagewall.png" alt="留言墙" />
                        </div>
                                ...
                                <script id="demo" type="text/html">
                                    ...
                                    <li>
                                        <div class="comment-parent">
                                            <img src="${base}/finance/static/blog/images/Absolutely.jpg" alt="absolutely" />
                                            <div class="info">
                                            ...
                                            <div class="comment-child">
                                                <fieldset class="layui-elem-field" style="border-color:#D0E9FF">
                                                    ...
                                                    <div class="layui-field-box">
                                                        ...
                                                        {{# }else{ }}
                                                        <img style="width: 40px;height: 40px" src="${base}/finance/static/images/face.jpg" alt="{{# if(item.updateUser.nickName != ''){ }} {{ item.updateUser.nickName }} {{# }else{ }} {{ item.updateUser.loginName }} {{# } }}  "  />
                                                        {{# } }}
                                                        <div class="info">
...
<!-- 底部 -->
<#include "${base}/finance/blog/common/foot.ftl">
<!-- 本页脚本 -->

<script src="${base}/finance/static/blog/js/about.js?v=1.0"></script>
<script type="text/javascript" src="${base}/finance/static/js/wangEditor.min.js"></script>
<script type="text/javascript" src="${base}/finance/static/js/xss.min.js"  ></script>
<script type="text/javascript" src="${base}/finance/static/js/tools.js?v=${.now?long}"  ></script>
<script>
    ...
    layui.use(['form','jquery','layer','flow','laytpl'],function(){
        ...
        var f = {
            ...
            isLazyimg: true,
                ...
                $.post("${base}/finance/showBlog/articleCommentList",{type:2,page:page},function (res) {
                    if(res.success){
        ...
        form.on('submit(formLeaveMessage)', function (data) {
            ...
            $.post("${base}/finance/showBlog/saveMessage",data.field,function(res){
                if (res.success){
        ...
        form.on('submit(formReply)', function (data) {
            ...
            $.post("${base}/finance/showBlog/replyMessage",data.field,function(res){
                if (res.success){
                    layer.msg("留言成功", { icon: 1 });
                    $(data.form).find('textarea').val('');
                    var html = '<div class="comment-child"><img src="${base}/finance/static/blog/images/Absolutely.jpg"alt="Absolutely"/><div class="info"><span class="username">Absolutely</span><span>' + data.field.content + '</span></div><p class="info"><span class="time">'+layui.laytpl.toDateString(res.data.createDate)+'</span></p></div>';
