...
<head>
    ...
    <!-- 比较好用的代码着色插件 -->
    <link href="${base}/finance/static/blog/css/prettify.css" rel="stylesheet" />
    <!-- 本页样式表 -->
    <link href="${base}/finance/static/blog/css/detail.css" rel="stylesheet" />
    <style type="text/css">
        .admincss img{
...
<nav class="blog-nav layui-header">
    <div class="blog-container">
        ...
        <a href="javascript:" class="blog-user layui-hide">
            <img src="${base}/finance/static/blog/images/Absolutely.jpg" alt="Absolutely" title="Absolutely" />
        </a>
        <!-- 不落阁 -->
                ...
                <@mychannel limit="5">
                    <#list result as item>
                        <li class="layui-nav-item <#if (item.href?contains('wzzl'))> layui-this</#if>" >
                            <a href="${base}/finance/showBlog${item.href}"><i class="layui-icon" style="font-size: 18px;">${item.logo}</i>&nbsp;${item.name}</a>
                        </li>
                    </#list>
                </@mychannel>
        </ul>
...
<!-- 比较好用的代码着色插件 -->
<script type="text/javascript" src="${base}/finance/static/blog/js/prettify.js"></script>
...
<script>
    layui.use(['form','jquery','layer','flow','laytpl'],function(){
        ...
        $(function(){
            $.post("${base}/finance/showBlog/click",{articleId:${article.id}},function (res) {
               if(!res.success){
        ...
        var f = {
            ...
            isLazyimg: true,
                ...
                $.post("${base}/finance/showBlog/articleCommentList",{articleId:${article.id},type:1,page:page},function (res) {
                    if(res.success){
        ...
        form.on('submit(formRemark)', function (data) {
            ...
            $.post("${base}/finance/showBlog/saveComment",data.field,function(res){
                if (res.success){
