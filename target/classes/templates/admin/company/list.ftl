    ...
    <div class="layui-field-box">
        <form class="layui-form">
            ...
            <div class="layui-inline" style="margin-left: 15px">
                <label>入驻企业名称:</label>
                <div class="layui-input-inline">
                    <input type="text" value="" name="s_type" placeholder="请输入企业名称"
            ...
            <div class="layui-inline" style="margin-left: 15px">
                <label>统一信用代码</label>
                <div class="layui-input-inline">
                    <input type="text" value="" name="s_hash" placeholder="请输入统一信用代码" class="layui-input search_input">
                </div>
            </div>
...
<script>
    layui.use(['layer', 'form', 'table'], function () {
        ...
        t = {
            ...
            cols: [[
                {type: 'checkbox'},
                {field: 'companyName', title: '入驻企业名称'},
                {field: 'creditCode', title: '统一信用代码'},
        ...
        table.on('tool(demo)', function (obj) {
            ...
            if (obj.event === 'edit') {
                var editIndex = layer.open({
                    title: "编辑字典",
                    type: 2,
                    content: "${base}/finance/company/queryCompany?id=" + data.id,
            ...
            if (obj.event === "del") {
                layer.confirm("你确定要删除该资源么？", {btn: ['是的,我确定', '我再想想']},
                    function () {
                        alert("1");
                        $.ajax({
                            url: "${base}/finance/company/deleteCompany",
                            type: 'post',
                            contentType: 'application/json',
                            data: JSON.stringify({"companyIds": data.id}),
                    ...
                    layer.confirm("你确定要删除这些资源么？", {btn: ['是的,我确定', '我再想想']},
                        function () {
                            ...
                            $.ajax({
                                ...
                                type: 'post',
                                contentType: 'application/json',
                                data: JSON.stringify({"companyIds": d.join(',')}),
                                success: function (res) {
                                    if (res.code == 0) {
                                        layer.msg("删除成功", {time: 1000}, function () {
                                            table.reload('test', t);
