...
<script>
    layui.use(['layer','form','table','laydate'], function() {
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑物品信息表",
                    type : 2,
                    content : "${base}/finance/admin/tGoods/edit?id="+data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该物品信息表么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/finance/admin/tGoods/delete",{"id":data.id},function (res){
                                if(res.success){
        ...
        var t = {
            elem: '#test',
            url:'${base}/finance/admin/tGoods/list',
            method:'post',
        ...
        var active={
            addTGoods : function(){
                var addIndex = layer.open({
                    title : "添加物品信息表",
                    type : 2,
                    content : "${base}/finance/admin/tGoods/add",
                ...
                if(data.length > 0){
                    ...
                    layer.confirm("你确定要删除这些用户么？",{btn:['是的,我确定','我再想想']},
                            function(){
                                ...
                                $.ajax({
                                    type:"POST",
                                    url:"${base}/finance/admin/tGoods/deleteSome",
                                    dataType:"json",
